...
import {
  ...
  parseDeckIndexSort,
  sortDecks,
} from '../../../src/site/deck-index-filter'
...
describe('sortDecks', () => {
  ...
  test("'alpha' sorts by name A-Z", () => {
    expect(sortDecks(decks, 'alpha', 'usd', false).map((d) => d.slug)).toEqual(['a', 'b', 'c', 'd'])
  })

  test("'recent' sorts newest first, undated last", () => {
    expect(sortDecks(decks, 'recent', 'usd', false).map((d) => d.slug)).toEqual([
      'a',
      'c',
      'b',
      'd',
  ...
  test("'price' sorts highest first", () => {
    expect(sortDecks(decks, 'price', 'usd', false).map((d) => d.slug)).toEqual(['a', 'c', 'b', 'd'])
  })

  test("'lowestPrice' sorts highest first", () => {
    expect(sortDecks(decks, 'lowestPrice', 'usd', false).map((d) => d.slug)).toEqual([
      'a',
      'c',
      'b',
      'd',
  ...
  test('reverse flips the result of any sort', () => {
    expect(sortDecks(decks, 'alpha', 'usd', true).map((d) => d.slug)).toEqual(['d', 'c', 'b', 'a'])
    expect(sortDecks(decks, 'price', 'usd', true).map((d) => d.slug)).toEqual(['d', 'b', 'c', 'a'])
  })

  test('does not mutate the input array', () => {
    const input = decks.slice()
    sortDecks(input, 'alpha', 'usd', true)
  ...
  test('price sort respects the selected currency', () => {
    ...
    expect(sortDecks([eurFavored, usdFavored], 'price', 'usd', false).map((d) => d.slug)).toEqual([
      'u1',
      'e1',
    ])
    expect(sortDecks([eurFavored, usdFavored], 'price', 'eur', false).map((d) => d.slug)).toEqual([
      'e1',
      'u1',
    ])
  })
...
describe('parseDeckIndexSort / parseDeckIndexGroup', () => {
  test('passes through valid sort values', () => {
    expect(parseDeckIndexSort('alpha')).toBe('alpha')
    expect(parseDeckIndexSort('recent')).toBe('recent')
    expect(parseDeckIndexSort('lowestPrice')).toBe('lowestPrice')
  })

  test('falls back to alpha for unknown sort strings', () => {
    expect(parseDeckIndexSort('totally-bogus')).toBe('alpha')
    expect(parseDeckIndexSort('')).toBe('alpha')
  })

  test('passes through valid group values', () => {
    expect(parseDeckIndexGroup('format')).toBe('format')
    expect(parseDeckIndexGroup('none')).toBe('none')
  })

  test('falls back to none for unknown group strings', () => {
    expect(parseDeckIndexGroup('unknown')).toBe('none')
  })
})
