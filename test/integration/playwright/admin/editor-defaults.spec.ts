...
test.describe('Admin Editor — Add Card Defaults', () => {
  ...
  test('defaults toggle is visible and collapsed by default with no defaults set', async ({
    page,
  }) => {
    const toggle = page.locator('.btn-defaults')
    await expect(toggle).toBeVisible()
    await expect(toggle).toContainText('Add Card Defaults')
    // No defaults active → no active indicator dot, and the panel stays collapsed.
    await expect(toggle.locator('.btn-defaults-dot')).toHaveCount(0)
    await expect(page.locator('.editor-action-defaults')).toHaveCount(0)
  })
  ...
  test('defaults persist across reload via localStorage', async ({ page }) => {
    ...
    // Reload and revisit the deck editor — defaults should persist. The action
    ...
    // Active defaults surface as a dot on the collapsed toggle; expand it and
    // confirm the Foil radio is still selected. The regex form keeps the match
    // case-sensitive so it does not also match "Nonfoil".
    const toggle = page.locator('.btn-defaults')
    await expect(toggle.locator('.btn-defaults-dot')).toBeVisible()
    await toggle.click()
