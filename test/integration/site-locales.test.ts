...
import { getRitualConfigPath, refreshRitualConfig } from '../../src/config/ritual-config'
...
describe('site locales (Integration)', () => {
  ...
  beforeEach(async () => {
    ...
    localeFile = path.join(ws.dir, `${PSEUDO}.json`)
  ...
  test('a malformed --locale-file fails the build and leaves the previous site published', async () => {
    ...
    const broken = path.join(ws.dir, 'de-AT.json')
