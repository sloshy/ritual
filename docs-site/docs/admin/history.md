...
The **Change History** page compacts and rewrites the change log (`.changes.md`) of a deck, collection, or wanted list from the browser. It mirrors the CLI [`history`](../commands/history.md) command — every edit is staged in memory and nothing is written until you save — and, like the command, it **only ever touches the change log**: the list's own `.md` file is never modified.
