...
This is the **CLI-driven** counterpart to the [`mcp`](./mcp.md) command: where `ritual mcp`
...
repository was set up with [`ritual init-site`](./init-site), upgrading it also refreshes any
installed skills automatically (see below).

## Installing during `init-site`

When you scaffold a repository for publishing with [`ritual init-site`](./init-site), it offers to
...
- [`init-site`](./init-site.md) — scaffolds a repository for publishing and can install these skills.
- [`mcp`](./mcp.md) — expose the same operations to MCP-native agents as tool calls.
- [`admin`](./admin.md) — the browser-based editor for the same lists.
