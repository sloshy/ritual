...
A command with no non-essential output does not register the flag at all rather than advertising an inert one. `card`, `diff`, `scry`, `cache status`, `dep-license`, `history`, `login status`, and `skills list` therefore accept `--output` but no `--quiet`: everything they print is either the payload or a warning that has to survive anyway.
