...
Where a question has a documented default, that default is used instead of failing — each command's page says which: `--refresh ask` is declined rather than answered (never a surprise multi-MB download), and a text `import` without `--type` falls back to a deck **under an explicit `--no-input`** (a merely piped run still errors, since nothing said which type was meant).
