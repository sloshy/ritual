...
Scalar forms are **rejected**, deliberately:
...
A `file` cover obeys the [custom-art reference rules](/custom-art/#the-sidecar) exactly — forward slashes, relative to the art directory, never escaping it, one of `.avif`, `.gif`, `.jpeg`, `.jpg`, `.png`, `.webp` — and is published the same way: [`build-site`](/commands/build-site/) copies it into `dist/art/`, and [`serve --api`](/commands/serve/) serves it live from the art directory.
...
A `url` cover is never validated — not when written, not at build time. A broken one is the browser's problem, exactly as for a custom-art URL. Note that `serve --api` bakes a `file` cover without checking it too, so a missing file there shows as a `404` in the page rather than a fallback.
...
A [`ritual edit`](/commands/edit/) session snapshots a list's front matter when it opens and re-emits the block on save. Two consequences, until the session grows its own cover action:
