...
Deck entries record the card name and quantity, plus the set code and collector number when pinned, and `[finish]`/`[condition]`/`[labels]` annotations when `--finish`/`--condition`/`--label` are given (`--label proxy` is the only label a deck line takes — see [Card Labels](/commands/edit/#card-labels)). A `--finish` on a deck add is validated only when `--set`/`--collector-number` pin a printing: an unpinned deck line records no printing, so there is nothing to validate the finish against.
