...
Deck entries record the card name and quantity, plus the set code and collector number when pinned, and `[finish]`/`[condition]`/`[labels]`/`#tags` annotations when `--finish`/`--condition`/`--label`/`--tag` are given. `--label proxy` is the only label a deck line takes (see [Card Labels](/commands/edit/#card-labels)); tags are free-form on every type (see [Card Tags](/commands/edit/#card-tags)). A `--finish` on a deck add is validated only when `--set`/`--collector-number` pin a printing, since an unpinned deck line records no printing to validate the finish against.
