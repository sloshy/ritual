...
- **`tags`** is the card's [tags](/list-format/#card-tags), comma-joined in canonical order (`Card Draw, Ramp`), the same spelling the editors use, without the `#` the card line writes (card-line punctuation, never part of a value). Every list type carries tags, and unlike labels there is no list-level default: a card's tags are exactly the ones on its line, so the column is empty for an untagged card. The `md` format writes them back inline as the line's `#tags` token on every list type. The spelling is the same in every dialect, since tags are Ritual-specific.
