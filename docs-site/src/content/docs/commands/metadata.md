...
Decks take `description`, `tags`, `format`, `sourceId`, and `sourceUrl`; collections take `labels` (their [default card labels](/commands/edit/#collection-front-matter)). Wanted lists define no front-matter keys, so every subcommand refuses them with a usage error. Writes go through the same engine as the admin [List Metadata](/admin/api/#list-metadata) route and the MCP `set_list_metadata` tool, so validation and the body-preserving write live exactly once: card lines — `&N` ids, label overrides, notes — survive byte for byte, and only the front-matter block is re-dumped (comments and quoting style are not preserved, though every key and value is).
...
| collection | `labels`      | Default card labels: `sale`/`trade` (combinable) or `keep` (exclusive), as separate values or comma-joined, case-insensitively. `--add`/`--remove` merge with the current set (a typo'd label errors rather than silently removing nothing; removing the last label clears the key). An empty `set` is refused — clearing is `unset`'s job. |
...
- **Validation matches the other surfaces**: label vocabulary and keep-exclusivity, deck format keys, `http(s)` source URLs, and the Archidekt id/URL agreement rule are all enforced exactly as the admin route enforces them.
