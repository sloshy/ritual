...
- **Card search** — adding cards searches Scryfall directly (preferring the shared session cache), the same as the Trade Planner. It matches [the same way the admin editor does](/admin/editors/#step-1-search) — every whitespace-separated term must appear in the name, in any order. Scryfall's own autocomplete only matches the query as one contiguous string, so when none of its suggestions begin their words with your terms (typing `in tre` for "In the Trenches"), the site re-asks Scryfall as a term search and leads with those results.
