...
- **Export your edits** — the **Export…** panel offers two ways to keep your changes:
  - **Download change list (JSON)** or **Copy JSON** — a portable change file that the admin editor can later import (see [Import changes](/commands/admin/#import-changes)) to apply your edits to the real list. Imported changes are re-targeted to the current card IDs and surfaced for review before saving.
