...
- **Text (.txt)** — for a deck, the [Moxfield export dialect](/commands/export/#dialects): bare `Commander` / `Deck` board markers over `N Card Name (SET) Collector Number` lines, with Moxfield's trailing `*F*` / `*E*` finish markers — no `##` headers, no `-` bullets, and none of the ids, notes, conditions or labels a Ritual line carries, so it pastes straight into Moxfield or Arena (extras are omitted). Collections and wanted lists render one line per card in Ritual's own quantity-prefixed form (`N Card Name (SET:Collector Number)`).
