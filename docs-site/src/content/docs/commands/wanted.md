...
| `🛠️ Switch to Edit Mode`             | Browse and edit the list's existing cards (see [Edit Mode](#edit-mode)) |
...
discard menus reset); `✅ Done` does the same and exits. `🚪 Exit Without Saving` throws away all
unsaved changes after a confirmation. Pressing <kbd>Esc</kbd>/<kbd>Ctrl-C</kbd> at the main prompt
behaves like `✅ Done`.
