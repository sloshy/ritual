...
Every Ritual command, grouped by what it works on. Each page follows the same shape: usage, options, examples, then the details. The conventions shared by all of them, such as global options, scripting output, and exit codes, are on [CLI Conventions](/cli-conventions/).
