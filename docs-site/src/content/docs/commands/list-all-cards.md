...
Print a deduplicated, alphabetically sorted manifest of every card in your decks, collections, and wanted lists. Its main job is to be a deterministic cache key for CI builds: the output changes only when the set of unique `(name, set, collector number)` tuples in your lists changes.
...
The cache is therefore reused across builds whenever the set of cards in your lists hasn't changed, even if you tweak finishes, conditions, notes, or quantities, or rearrange existing cards across files.
