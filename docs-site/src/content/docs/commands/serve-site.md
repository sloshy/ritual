...
Use this instead of running [`build-site`](/commands/build-site/) and [`serve`](/commands/serve/) separately when you want a one-shot preview.
...
The build options mirror [`build-site`](/commands/build-site/); two additional options control the serving side.
...
| `--refresh <mode>`          | Card cache refresh policy: `ask` (prompt; skip when prompts are unavailable), `auto`, `no-bulk`, or `never` (see [build-site](/commands/build-site/#card-cache-refresh)). | `ask`     |
...
- For details on individual build options (themes, currencies, output format, trade planner, etc.), see [`build-site`](/commands/build-site/).
- To serve a previously built `dist/` without rebuilding, use [`serve`](/commands/serve/).
- For an auto-restarting workflow that rebuilds when source or data files change, see [Development → Dev Workflow](/development/#dev-workflow). Note that `bun run dev serve-site` requires an explicit `--refresh` mode (`auto`, `no-bulk`, or `never`) so the cache refresh prompt can be answered non-interactively.
