...
`list` deliberately has **no `--quiet`**: its entire output is the skill list itself, and the [shared convention](/cli-conventions/#scripting-conventions) is that `--quiet` never suppresses the payload — so there would be nothing for the flag to do. Redirect stdout if you want silence.
