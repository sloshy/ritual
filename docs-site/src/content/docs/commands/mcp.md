...
entry without its own override inherits (`null` clears them). All three list types also take
...
and a `url` is never validated at all. `image` is the **only** field a wanted list accepts (use
`rename_list` to change its display name); a `labels` key on a wanted body is refused by name. Setting `sourceId` together with an `archidekt.com`
