...
Cleanup never adds changelog entries — a cleaned-up file has the same cards it had before. Two cases are reported with a warning instead of fully acted on: a rename whose target file name is already taken by another list, and a file whose parse skipped any lines — malformed card lines, but also prose, comments, or any other text the list grammar does not model. In the second case the file is still renamed if its name drifted, but its content is left alone (rewriting it would silently drop those lines; fix or remove them and rerun).
