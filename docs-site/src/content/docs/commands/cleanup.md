...
Normalize every deck, collection, and wanted list file in the workspace to the current conventions. Run it after hand-editing files, importing from older tools, or upgrading a workspace whose files predate a convention change (for example, deck files that were still named in lower-kebab-case).
...
A file cleanup cannot read at all (broken YAML front matter, bad permissions) is reported by name and **skipped**. Nothing about it is rewritten or renamed, every other list is still cleaned up, and the run exits 1. Hand-edited workspaces are the case cleanup exists for, so one unparseable file never aborts the pass:
