...
The name is matched case- and accent-insensitively across all three list types, with a unique-substring fallback. An ambiguous name is rejected; disambiguate with `--deck`, `--collection`, or `--wanted`. See [List Names](/list-resolution/).
...
Selecting a list opens a card browser over that list. **🔎 Search all cards** opens the same browser over every list at once, with each card labelled by its source list. In a card browser, typing filters rows live by name, set code, or collector number, and menu items adjust the sort field and direction and set persistent set-code, collector-number, and (in the global search) list-type filters. Selecting a card shows a detail view with its printing, unit and line price, the cheapest printing, mana value, and EDHREC rank, and can list every printing with per-finish prices.
