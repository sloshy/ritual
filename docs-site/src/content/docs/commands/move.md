...
- **Deck files are rewritten in canonical form**: a move touching a deck re-serializes that deck file, so lines its parser cannot read (prose, comments, empty sections) are dropped. Collection and wanted-list files are edited as text and keep such lines.
