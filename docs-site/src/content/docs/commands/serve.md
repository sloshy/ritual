...
The two buylist routes are **not** what the public site reads: it gets its buy prices baked into the live list payloads (below). They remain mounted for other clients — a script, a second front end — and answer `404` when neither sell mode nor the `cardkingdom` [price store](/configuration/#price-stores-pricesources) is on.
