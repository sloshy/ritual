...
| GET    | `/api/buylist/status`                                    | Which buyers this server can quote against and how fresh its cached buylist is. Powers [sell mode](/public-site/sell/); `404` when `site.sellMode` is `false`. |
...
With `--build`, `serve` accepts the full [`build-site`](/commands/build-site/) option surface. Passing any of these **without** `--build` is a usage error: the command exits with code 2 and an error naming the offending flag(s). Three exceptions: `--refresh`, which is also meaningful with `--api` (cache warming); `--out-dir`, which names the directory to serve whether or not a build runs; and `--locale`, which is a [global flag](/#global-options) every command accepts.
