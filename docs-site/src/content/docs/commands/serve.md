...
| GET    | `/api/buylist/status`                                    | Which buyers this server can quote against and how fresh its cached buylist is. `404` unless [sell mode](/public-site/sell/) is on.                       |
...
The two buylist routes are **not** what the public site reads: it gets its buy prices baked into the live list payloads (below). They remain mounted for other clients — a script, a second front end — and answer `404` when sell mode is off.
