...
The **Find** page searches every list on a public site for a list of cards of your own. Paste in the names of the cards you're looking for and Find shows which of them appear in which lists. It is handy when you're browsing someone else's collection and want to compare it against a list of cards you have or want.
...
- Each list type (**Decks**, **Collections**, **Wanted Lists**) has a checkbox that includes or excludes every list of that type at once. Untick **Decks**, say, to search only collections and wanted lists. The tally next to the name shows how many of the type's lists are in scope (`3/5`).
...
This makes it easy to refine the leftover names, fix a typo or try an alternate spelling, and search again.
...
- **Add Selected to Trade** adds every selected card to the active [trade](/public-site/trade/), each carrying its original source list (collection/deck cards go to the left side, wanted cards to the right). Name-only cards prompt for a printing, just like elsewhere.
