...
- **Sell mode quotes refresh without a rebuild.** [Sell mode](/public-site/sell/) — Card Kingdom buylist prices beside each card, buylist filters (on-buylist chips and a price threshold), buylist grouping and sorting, and a sell-cart export — works on a static site too, since the buy prices are baked into each list's JSON. What hosting adds is currency: a live server bakes them per request, so a refreshed feed shows up on the next page load rather than at the next build. It is off unless [`site.sellMode`](/configuration/#offering-sell-mode-sellmode) is on or the run passed `--sell-mode`. The server refreshes a day-old feed once, at startup — never on a request — so a long-running deployment restarts into current offers. It never downloads the _first_ feed, though: for that, run `ritual sell --refresh auto` or use the admin site.
