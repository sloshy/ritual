...
When [sell mode](/public-site/sell/) is enabled (the **Offer sell mode** checkbox on the [Settings](/commands/admin/#settings) page, `site.sellMode`, or [`ritual admin --sell-mode`](/commands/admin/#sell-mode) — it is off by default), the toolbar's **Sell mode** toggle works here too, so you can pick out what a buyer will take before moving it.
