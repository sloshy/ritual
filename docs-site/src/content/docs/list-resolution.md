...
Ritual searches the existing list files and applies these rules in order:

1. **Byte-exact match.** A list whose file name equals the input character for character wins outright. Only a trailing `.md` and surrounding whitespace are removed first. Typing a name exactly always selects that one list.
...
Tiers 2 and 3 compare the query and each file name in a folded form. Exactly these things are ignored:
...
The last two rows make a **display name round-trip**. A list created as `Atraxa: Praetors' Voice` is stored as `Atraxa Praetors' Voice.md`, because a colon cannot be in a file name, and the name you typed at creation still finds it. Resolution folds the query through the same function the [file namer](/commands/new/#list-file-names) uses, so the two never disagree.

Two lists whose names differ only in folded characters (`Mono Red` and `mono-red`, or `AB.md` beside `A'B.md`) are ambiguous at tiers 2 and 3. Each is still reachable by tier 1: type its full name exactly as the file spells it. Ritual refuses to create the second name of such a pair, so one can exist only in an older workspace. [`rename`](/commands/rename/) moves one of them out of the way.

### Names that would collide are refused at creation

Folding makes `Atraxa Superfriends` and `atraxa superfriends` the same name, so [`new`](/commands/new/) and [`rename`](/commands/rename/) refuse a name that folds onto an existing list of the same type:

```
A deck named 'Atraxa Superfriends' already exists (it matches 'atraxa superfriends' under list-name folding).
```

The refusal is a usage error (exit `2`; HTTP `409` on the admin API) and names the existing list. Renaming a list to a different spelling of **its own** name (fixing capitalization, dropping a colon) is allowed.
...
Most commands search **all three** list types at once: `add-card`, `remove-card`, `set-card`, `note`, `edit`, `history`, `price`, `sell`, `export`, `rename`, and `delete`. A name that exists in more than one type (a deck _and_ a collection both called `staples`) is ambiguous. Resolve it with a type flag:
...
The flags are mutually exclusive. A `deck:`/`collection:`/`wanted:` prefix on the name itself (`collection:staples`) selects the type too, and supplies it when no flag is given. A prefix that **contradicts** the flag is a usage error (exit `2`) naming both:
...
Commands that take **more than one** list cannot scope each argument with one whole-command flag, so their ambiguity errors suggest the prefix. That covers [`diff`](/commands/diff/)'s two sides, [`move`](/commands/move/)'s `--from`/`--to`, and the list arguments of [`export`](/commands/export/) and [`sell`](/commands/sell/). [`lists`](/commands/lists/) does not resolve a name, but accepts the same three flags to filter which types it enumerates.

Single-type commands (`deck-sync`, `collection-sync`, `get-primer`, and the CSV importer) already know their type, so they take no flag. They match names by the same folding, substring, and ambiguity rules. Their ambiguity errors never suggest a type selector: they use the first row of the table below, or the second when the matching names fold together.
...
A type selector cannot break a tie between two lists of the same type, so the same-type case always asks for a longer name. A type is only suggested when it holds exactly **one** match. When no type qualifies, the error asks for a longer name instead.

The suggested example name always resolves. When two files fold to the same name (`Storm Crow.md` and `storm-crow.md`), a longer name would not help, so the advice asks for one list's exact full name, which the byte-exact tier honors. Only when the matches are byte-identical (the same file name under two list types) is no example offered, because no name can break that tie.
