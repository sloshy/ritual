...
Each list's art lives beside it as `<list>.art.json`, so `decks/Winota Stax.art.json` sits next to `decks/Winota Stax.md`. It is a JSON object keyed by the card line's [`&N` id](/cli-conventions/#the-card-id-backfill), whose values carry exactly one of `file` or `url`:
...
- **`url`** must be an absolute `http`/`https` URL, and is used verbatim. No extension rule applies here, since image URLs routinely end in a query string or nothing at all, and the browser decides what it can render.
...
A malformed sidecar fails **as a whole** rather than loading the entries it could read, because a partial read would be erased by the next write. A bad file is reported and the list still loads, editors included; its cards just show their normal art. The message names the exact problem: `card 5: "..\\secret.png" escapes the art directory`, `"x" is not a card id — keys are the card line's &N`, `not valid JSON: …`.
...
Every write is a read-modify-write of the whole sidecar, and each refuses outright when the existing file cannot be read, since overwriting it would erase art for cards the request never mentioned.
...
Only the **front** image is replaced. A double-faced card's back keeps its real face, since the override is the entry's, not the printing's. The modal's **Other Printings** grid and the editors' printing pickers also keep real thumbnails: they exist to show you actual printings.
