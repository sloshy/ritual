...
/**
 * localStorage namespace for editor defaults. The admin and public editors run on
 * different origins, but use distinct namespaces anyway so the key reads correctly
 * in each context ('site' for the public editor, 'admin' for the admin server).
 */
export type EditorDefaultsNamespace = 'site' | 'admin'

const storageKey = (namespace: EditorDefaultsNamespace, kind: EditorListKind): string =>
  `ritual:${namespace}:defaults:${kind}`
...
function loadStored(kind: EditorListKind, namespace: EditorDefaultsNamespace): EditorDefaults {
  if (typeof localStorage === 'undefined') return emptyDefaults(kind)
  const result = parseStored(localStorage.getItem(storageKey(namespace, kind)), kind)
...
/**
 * Add-card defaults (set filter, finish, condition) persisted to localStorage.
 * `namespace` keeps the public editor's key ('site') distinct from the admin
 * server's ('admin'); it defaults to 'site' so the public editor needs no extra
 * argument.
 */
export function useEditorDefaults(
  kind: EditorListKind,
  namespace: EditorDefaultsNamespace = 'site',
): UseEditorDefaultsResult {
  const [defaults, setDefaultsSignal] = createSignal<EditorDefaults>(loadStored(kind, namespace))
  ...
  createEffect(() => {
    ...
    localStorage.setItem(storageKey(namespace, kind), JSON.stringify(payload))
  })
