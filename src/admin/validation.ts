 ...
 *
 * `MAX_BODY_SIZE` (`src/api/http.ts`) put an arbitrary ceiling of a few dozen
 * changes on a single save — re-pinning the printing of every card in a
 * commander deck is one ordinary edit that sends ~100 changes — and reported it
 * only as "Request body too large".
 *
